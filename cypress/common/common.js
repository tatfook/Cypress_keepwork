...
function createSiteorPage(selectors,data){
...
function verifySearchContent(selector, expected){
  cy.get(selector).should(($val) => {
    expect($val[0].innerText).to.be.eql(expected)
  })
}
...
module.exports = { GenStr, login, logout, signup, search, verifySearchTotalNum, verifySearchContent, filterby, createSiteorPage}